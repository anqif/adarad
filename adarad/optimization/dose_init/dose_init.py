...
def dyn_init_dose(A_list, alpha, beta, gamma, h_init, patient_rx, T_recov = 0, use_dyn_slack = False, slack_dyn_weight = 0,
    ...
    # Stage 1: Solve static (convex) seq_cvx in initial session.
    if init_verbose:
        print("Stage 1: Solving static seq_cvx in session {0}".format(t_static))
    ...
    if init_verbose:
        print("Stage 2a: Solving dynamic seq_cvx for best constant beam scaling factor")
    ...
    # Stage 2b: Solve dynamic (nonconvex) seq_cvx with scaled static beams.
    if init_verbose:
        print("Stage 2b: Solving dynamic seq_cvx for best time-varying beam scaling factors")
