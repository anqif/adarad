...
# Beam subproblems.
def run_beam_proc(pipe, A, beam_lower, beam_upper, dose_upper, dose_lower, rho):
	K, n = A.shape
	...
	# Form dose penalty and constraints.
	...
	while not finished:
		...
		# Send optimal dose and (setup + solve) time.
...
def main():
	...
	for t in range(T):
		local, remote = Pipe()
		pipes += [local]
		procs += [Process(target=run_beam_proc, args=(remote, A_list[t], beam_lower[t], beam_upper[t], dose_upper[t], dose_lower[t], rho))]
		procs[-1].start()
	...
	while not finished:
		...
		for t in range(T):
			pipes[t].send((d_tld_var_val[t], u_val[t]))
		...
		# Form health subproblem objective.
		c_penalty = (rho/2.0) * sum_squares(d_tld - d_var_val + u_val)
		obj = h_penalty + s_penalty + c_penalty

		# Solve health subproblem using CCP.
		obj_old = np.inf
		d_tayl_val = d_tld_var_val_old  # TODO: What dose point should we linearize PTV health dynamics around?
		for l in range(max_iter_ccp):
			...
			setup_time = 0 if prob_h.solver_stats.setup_time is None else prob_h.solver_stats.setup_time
			total_time_admm += setup_time + prob_h.solver_stats.solve_time

			# Terminate if change in objective is small.
			obj_diff = obj_old - prob_h.value
			# print("ADMM CCP Iteration {0}, Objective Difference: {1}".format(l, obj_diff))
			if obj_diff <= eps_ccp:
				break
			obj_old = prob_h.value
			d_tayl_val = prob_h_dict["d_tld"].value

		d_tld_var_val_old = d_tld_var_val
		d_tld_var_val = prob_h_dict["d_tld"].value

		# Update dual values.
		u_val = u_val + d_tld_var_val - d_var_val

		# Calculate residuals.
		r_prim = d_var_val - d_tld_var_val
		r_dual = rho*(d_tld_var_val - d_tld_var_val_old)
		...
		eps_dual = eps_abs*np.sqrt(T*K) + eps_rel*LA.norm(u_val)
