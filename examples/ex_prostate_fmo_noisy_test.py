...
def main(figpath = "", datapath = ""):
	# Import data.
	figprefix = "ex_prostate_FMO_stanford_noisy_test-"
	...
	plot_health(res_mpc["health"], curves = h_curves, stepsize = 10, bounds = (health_lower, health_upper),
				label = "Treated (MPC)", color = colors[2], one_idx = True, filename = figpath + figprefix + "mpc-health.png")
	plot_treatment(mpc_doses_act, curves = d_curves, stepsize = 10, bounds = (dose_lower, dose_upper),
				label = "MPC", color = colors[2], one_idx = True, filename = figpath + figprefix + "mpc-doses.png")
