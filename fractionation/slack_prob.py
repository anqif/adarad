...
from fractionation.mpc_funcs import dyn_objective
...
def rx_to_slack_constrs(expr, rx_dict, slack_lo, slack_hi):
...
# Construct optimal control problem with slack penalty and constraints.
def build_dyn_slack_prob(A_list, F_list, G_list, r_list, h_init, patient_rx, T_recov=0, slack_final=False):
    ...
    s_vars = [b_slack_lo, b_slack_hi, h_slack_lo, h_slack_hi, d_slack_lo, d_slack_hi]
    ...
    if "beam_constrs" in patient_rx:
        constrs += rx_to_slack_constrs(b, patient_rx["beam_constrs"], b_slack_lo, b_slack_hi)

    # Additional dose constraints.
    if "dose_constrs" in patient_rx:
        constrs += rx_to_slack_constrs(d, patient_rx["dose_constrs"], d_slack_lo, d_slack_hi)

    # Additional health constraints.
    if "health_constrs" in patient_rx:
        constrs += rx_to_slack_constrs(h[1:], patient_rx["health_constrs"], h_slack_lo, h_slack_hi)
    ...
    if T_recov > 0:
        ...
        s_vars += [h_r_slack_lo, h_r_slack_hi]
        ...
        if "recov_constrs" in patient_rx:
            constrs_r += rx_to_slack_constrs(h_r, patient_rx["recov_constrs"], h_r_slack_lo, h_r_slack_hi)
        constrs += constrs_r

    # Disable slack for final session.
    if not slack_final:
        constrs += [b_slack_lo[-1] == 0, b_slack_hi[-1] == 0, d_slack_lo[-1] == 0, d_slack_hi[-1] == 0]
        if T_recov > 0:
            constrs += [h_r_slack_lo[-1] == 0, h_r_slack_hi[-1] == 0]
        else:
            constrs += [h_slack_lo[-1] == 0, h_slack_hi[-1] == 0]

    # Objective function.
    obj = dyn_slack_objective(d, h, patient_rx, s_vars)
